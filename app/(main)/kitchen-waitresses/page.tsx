...
import { Plus, Pencil, Trash2, UserRound, Phone, CalendarClock } from "lucide-react";
...
import type { IKitchenWaitress } from "@/types";

async function fetchKitchenWaitresses(): Promise<IKitchenWaitress[]> {
  const res = await fetch("/api/kitchen-waitresses");
  if (!res.ok) throw new Error("fetch");
  const data = (await res.json()) as IKitchenWaitress[];
  return data.map((w) => ({ ...w, isActive: w.isActive !== false }));
}

function KitchenWaitressDialog({
  open,
  onClose,
  waitress,
}: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = waitress ? `/api/kitchen-waitresses/${waitress._id}` : "/api/kitchen-waitresses";
    ...
    toast({
      variant: "success",
      title: waitress ? "Serveuse-cuisinière modifiée" : "Serveuse-cuisinière ajoutée",
    });
    qc.invalidateQueries({ queryKey: ["kitchen-waitresses"] });
    onClose();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{waitress ? "Modifier la serveuse-cuisinière" : "Nouvelle serveuse-cuisinière"}</DialogTitle>
        </DialogHeader>
...
export default function KitchenWaitressesPage() {
  const PAGE_SIZE_OPTIONS = [5, 10, 20, 50, 100] as const;
  const { data: session, status } = useSession();
  const qc = useQueryClient();

  const canManageKitchenWaitresses = ["directeur", "directrice"].includes(session?.user?.role ?? "");
  const isDirector = session?.user?.role === "directeur";
  const { data: waitresses, isLoading } = useQuery({
    queryKey: ["kitchen-waitresses"],
    queryFn: fetchKitchenWaitresses,
    enabled: canManageKitchenWaitresses,
  });

  const [dialogOpen, setDialogOpen] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<(typeof PAGE_SIZE_OPTIONS)[number]>(10);
  const [edit, setEdit] = useState<IKitchenWaitress | undefined>();
  const [waitressPendingDelete, setKitchenWaitressPendingDelete] = useState<IKitchenWaitress | null>(null);
  const [waitressToggleConfirm, setKitchenWaitressToggleConfirm] = useState<{
    waitress: IKitchenWaitress;
    nextActive: boolean;
  } | null>(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/kitchen-waitresses/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error((await res.json()).error);
    },
    onSuccess: () => {
      toast({ variant: "success", title: "Serveuse-cuisinière supprimée" });
      qc.invalidateQueries({ queryKey: ["kitchen-waitresses"] });
      setKitchenWaitressPendingDelete(null);
    },
    onError: (err: Error) => {
  ...
  const toggleMutation = useMutation({
    mutationFn: async ({ id, nextActive }: { id: string; nextActive: boolean }) => {
      const res = await fetch(`/api/kitchen-waitresses/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      return body as IKitchenWaitress;
    },
    onSuccess: (_, { id, nextActive }) => {
      setKitchenWaitressToggleConfirm(null);
      qc.setQueryData<IKitchenWaitress[]>(["kitchen-waitresses"], (old) =>
        old?.map((w) =>
      ...
      toast({
        variant: "success",
        title: nextActive ? "Serveuse-cuisinière réactivée" : "Serveuse-cuisinière désactivée",
  ...
  const openEdit = (w: IKitchenWaitress) => {
    setEdit(w);
    setDialogOpen(true);
  };

  const count = waitresses?.length ?? 0;
  const paginatedKitchenWaitresses = (waitresses ?? []).slice((currentPage - 1) * pageSize, currentPage * pageSize);
  ...
  return (
    <div>
      <PageHeader
        title="Serveuses-Cuisinières"
        subtitle="Équipe de service cuisine"
        action={
          <Button onClick={openCreate}>
            <Plus className="h-4 w-4" />
            Ajouter une serveuse-cuisinière
          </Button>
        }
      />
      ...
      <div className="mb-10 max-w-xs">
        {isLoading ? (
          ...
          <StatsCard title="Serveuses-cuisinières" value={count} icon={UserRound} index={0} />
        )}
      </div>
      ...
      >
        ...
        <PremiumTableShell
          ...
          emptyMessage="Aucune serveuse-cuisinière enregistrée"
          emptyAction={
          ...
          mobileContent={
            <MobileCardList>
              {paginatedKitchenWaitresses.map((w) => (
                <MobileDataCard key={w._id}>
                  ...
                  <MobileDataCardActions>
                    <Button
                      ...
                      onClick={() =>
                        setKitchenWaitressToggleConfirm({
                          waitress: w,
                    ...
                    {isDirector && (
                      <Button
                        ...
                        onClick={() => setKitchenWaitressPendingDelete(w)}
            ...
            <table className="w-full min-w-[720px] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200/70 bg-slate-950/[0.025] text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">
                  <th className="whitespace-nowrap px-6 py-3.5 font-semibold">Serveuse-cuisinière</th>
      ...
      <KitchenWaitressDialog open={dialogOpen} onClose={() => setDialogOpen(false)} waitress={edit} />

      <Dialog
        open={!!waitressToggleConfirm}
        onOpenChange={(open) => {
          if (!open && !toggleMutation.isPending) setKitchenWaitressToggleConfirm(null);
        }}
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {waitressToggleConfirm?.nextActive ? "Réactiver la serveuse-cuisinière" : "Désactiver la serveuse-cuisinière"}
            </DialogTitle>
            <DialogDescription>
          ...
          <DialogFooter className="gap-2">
            <Button
              ...
              variant="outline"
              disabled={toggleMutation.isPending}
              onClick={() => setKitchenWaitressToggleConfirm(null)}
            >
              Annuler
            </Button>
            <Button
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && !deleteMutation.isPending && setKitchenWaitressPendingDelete(null)}
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Supprimer la serveuse-cuisinière ?</DialogTitle>
            <DialogDescription>
              {waitressPendingDelete && (
                <>
                  ...
                  . Cette action est irréversible. Si des commandes sont liées à cette serveuse-cuisinière, désactivez-la
          ...
          <DialogFooter className="gap-2">
            <Button
              type="button"
              variant="outline"
              onClick={() => setKitchenWaitressPendingDelete(null)}
